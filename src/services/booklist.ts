...
const addBookToList = async (req: Request, res: Response, next: NextFunction) => {
  const { username } = res.locals;
  const { isbn, title, author, publisher, pages, year, status, score, start_date, end_date } = req.body;
  const book: Book = {
    isbn,
    title,
    author,
...
const searchBook = async (req: Request, res: Response, next: NextFunction) => {
  const queryString: string = String(req.query.query);
  res.status(200).json(success([{ name: "query", message: queryString }]));
};

export { getBookById, getFullList, fetchListByStatus, addBookToList, updateBook, deleteBook, searchBook };
